 ...
 */
class Q_Pricespercustomer_Model_Observer
{
    /**
     * @param $observer
     * @return $this
    ...
    protected function _adjustPrice($adjustment = array(), $price)
    {
        if (array_key_exists('price_adjustment', $adjustment) && $adjustment['price_adjustment']) {
            ...
            } elseif ($adjustment['price_adjustment_type'] === 'Percent' || $adjustment['price_adjustment_type'] === 'percent') {
    ...
    protected function getCustomerPrices($cid, $pid)
    {
        ...
        $qPricePerCustomer = $readConnection->select()
            ->from(array('qppc' => $resource->getTableName('q_pricespercustomer/entity')))
            ->where("customer_id = ?", $cid)
            ->where("product_id = ?", $pid)
            ->order('created_at ASC')
        ;

        $pricePerCustomerCollection = $readConnection->fetchAll($qPricePerCustomer);
